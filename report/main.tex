...
generative prior coupled with a differentiable physics surrogate.\footnote{Code available at \url{https://github.com/pranavM2703/APL744-Project}}  The
...
This work establishes a complete, modular pipeline connecting three
distinct components:
\begin{enumerate}[nosep]
  \item A \textbf{generative prior} (MicroDiT$_{\text{XL/2}}$ or lightweight
    substitute) that constrains inverse-designed microstructures to be
    physically realistic.
  \item A \textbf{differentiable physics surrogate} ($f_\theta$: ResNet-18)
    that maps latent codes to effective stiffness with verified gradient flow.
  \item A \textbf{gradient-clipped SDS loop} that balances manifold adherence
    against property matching.
\end{enumerate}
