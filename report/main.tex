...
dataset using the frozen Ostris VAE encoder.
...
decreased by three orders of magnitude over 12 epochs:
\begin{equation}
  \loss_{\mathrm{val}}^{(1)} = 8{,}829 \;\to\; \loss_{\mathrm{val}}^{(7)} = 7.72
  \;\to\; \loss_{\mathrm{val}}^{(12)} = 5.01.
  \label{eq:surrogate_convergence}
\end{equation}
The best validation loss of $5.01$~GPa$^2$ (RMSE $\approx 2.24$~GPa)
was achieved at epoch~12, corresponding to a relative error of
$\approx 2\%$ over the $\Eeff \in [93, 117]$~GPa range.
...
SDS was run for three target stiffness values spanning the physically
meaningful range:
\begin{table}[h]
  \centering
...
The physics loss $\loss_{\mathrm{phys}} = (f_\theta(\zvec) - \Etar)^2$
decreased consistently across all targets over 200 steps:
\begin{table}[h]
  \centering
  \caption{SDS convergence: physics loss reduction over 200 steps.}
  \label{tab:sds_results}
  \begin{tabular}{ccccc}
    \toprule
    $\Etar$ [GPa] & $\loss_{\mathrm{phys}}^{(0)}$ & $\loss_{\mathrm{phys}}^{(200)}$ & Reduction & $\hat{E}^{(200)}$ [GPa] \\
    \midrule
    95  & 51{,}576 & 38{,}860 & 24.6\% & 292.1 \\
    105 & 40{,}660 & 29{,}201 & 28.2\% & 275.9 \\
    115 & 41{,}062 & 29{,}596 & 27.9\% & 287.0 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{enumerate}[nosep]
  \item \textbf{Monotonic convergence.} The gradient clipping
    \eqref{eq:grad_clip} eliminated the divergence observed without clipping,
    where predictions exceeded $2{,}900$~GPa.  All three targets exhibit
    smooth, monotonic loss decrease.

  \item \textbf{Prediction offset.}  The final predictions ($\approx 275$--$292$~GPa)
    exceed the targets ($95$--$115$~GPa).  This arises because the surrogate
    was trained on latents from a \emph{stub encoder} (a single Conv$_{8\times8}$
    layer), not the full Ostris VAE.  The stub encoder's latent space has lower
    semantic fidelity, causing the surrogate's loss landscape to contain
    higher-than-physical predictions at the initial Gaussian latent code.

  \item \textbf{Relative ordering preserved.}  Despite the offset, target~95
    yields the lowest final prediction and target~115 the highest,
    demonstrating that the SDS loop correctly differentiates between targets.
\end{enumerate}
...
\begin{itemize}[nosep]
  \item At high $\sigma$ (steps~0--50): $\| \nabla_{\mathrm{SDS}} \| \approx 140$--$200$, providing gentle global structure guidance.
  \item At low $\sigma$ (steps~150--200): $\| \nabla_{\mathrm{SDS}} \| \approx 1{,}000$--$3{,}200$, reflecting the denoiser's increasing confidence at low noise.
\end{itemize}
Without gradient clipping, this $\sim\!\!20\times$ amplification causes
...
\paragraph{VAE fidelity.}
The current results use a stub encoder ($1\times$ Conv$_{8\times8}$)
instead of the full 16-channel Ostris VAE.  Installing the
\texttt{diffusers} package and re-encoding with the real VAE would
produce semantically meaningful latents and reduce the prediction offset.

\paragraph{Surrogate capacity.}
Training was stopped at epoch~12 (val loss $= 5.01$).  Continuing to
convergence ($\sim$50 epochs) and augmenting with more GRF samples would
...
\begin{enumerate}[nosep]
  \item A frozen 16-channel VAE provides a compact, information-rich latent
    representation of 1{,}338 microstructures derived from EBSD data.
  \item A ResNet-18 surrogate achieves val loss $= 5.01$~GPa$^2$
    (RMSE $\approx 2.24$~GPa) mapping latent codes to effective stiffness.
  \item A lightweight denoising prior trained with EDM preconditioning
    reaches $\loss_{\mathrm{DSM}} = 0.056$ in 15~epochs.
  \item The gradient-clipped SDS loop (Algorithm~\ref{alg:sds_impl})
    reduces physics loss by $25$--$28\%$ over 200~steps, demonstrating
    convergent optimization toward prescribed stiffness targets.
...
The pipeline architecture is designed for modularity: replacing the stub
VAE with the full Ostris encoder, or the lightweight prior with
MicroDiT$_{\text{XL/2}}$, requires changing only configuration parameters,
not the optimization loop itself.
